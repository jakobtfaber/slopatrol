...
**User**:
A person with an identity in one Slopatrol organization — may run coding agents, review sessions, administer the Server, or any combination.
...
- **Listeners** and the **Server** communicate by exchanging **Events** defined in `@slopatrol/events`.
...
- "user" vs "developer" — resolved: the entity is **User**; "developer" is a role (the subset of **Users** who run coding agents observed by Slopatrol). The prototype leaderboard column labelled "Developer" is a role-filtered view of **Users**, not a separate entity.
- "wire" vs "events" — resolved: the Listener↔Server channel is described in terms of **Events**, not "wire." `packages/wire` has been renamed `packages/events` (package name `@slopatrol/events`). The TypeScript type stays `NormalEvent` to avoid colliding with the global DOM `Event` type.
